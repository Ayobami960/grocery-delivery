...
const sendMonthlyOffers = inngest.createFunction({
    ...
    // Send in baches of 10 to avoid overwhelming mail server
    const batchSize = 10;
    for (let i = 0; i < users.length; i += batchSize) {
        ...
        await step.run(`Send-offers-batch-${i}`, async () => {
            for (const u of batch) {
                await sendEmail({
                    to: u.email,
                    subject: `Fresh Picks Just Fo You!`,
                    body: `
                        <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 520px; margin: auto; border: 1px solid #e5e7eb; border-radius: 16px; overflow: hidden;">
...
// Auto-Assign Rider aofter 5 Minutes
const autoAssignRider = inngest.createFunction({
    ...
    const result = await step.run("assign-rider", async () => {
        ...
        // Skip if order doesn't exist, already assigned, or canceled
        ...
        // Find an actie rider not currently delivering
        ...
        const availableRider = await prisma.deliveryPartner.
            findFirst({
                where: {
                    isActive: true,
...
export const functions: ReturnType<typeof inngest.createFunction>[] = [checkLowStock, sendMonthlyOffers, autoAssignRider];